...
class Preprocessor:
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None, vocab_dict: Dict = None) -> Dict:
        """
        ...
        # step 7: co-expression network
        # if self.threshold:
        #     logger.info("Constructing co-expression network ...")
        #     if not isinstance(self.threshold, float):
        #         raise ValueError(f"Threshold must be a float, got {type(self.threshold)}")
        #
        #     if self.threshold < 0. or self.threshold > 1.:
        #         raise ValueError(f"Threshold must be between 0-1, got {self.threshold}")
        #
        #     if not isinstance(self.n_neighbours, int):
        #         raise ValueError(
        #             "Neighbours must be an integer, but got {}.".format(self.n_neighbours)
        #         )
        #     gene_list = adata.var[self.gene_col].values.tolist()
        #     idx2gene = dict(zip(range(len(gene_list)), gene_list))
        #     out = self.np_pearson_cor(adata.X, adata.X)  # (gene_num, gene_num)
        #     out[np.isnan(out)] = 0
        #     out = np.abs(out)
        #     out_sort_idx = np.argsort(out)[:, -(self.n_neighbours + 1):]
        #     out_sort_val = np.sort(out)[:, -(self.n_neighbours + 1):]
        #     df_g = []
        ...
        #             # source = out_sort_idx[i, j]
        #             df_g.append((source, target, out_sort_val[i, j]))
        #
        #     df_g = [i for i in df_g if i[2] > self.threshold]
        #     co_df = pd.DataFrame(df_g).rename(columns={0: 'source', 1: 'target', 2: 'importance'})
        #     co_df['source'] = co_df['source'].map(vocab_dict)
        #     co_df['target'] = co_df['target'].map(vocab_dict)
        #     return co_df
