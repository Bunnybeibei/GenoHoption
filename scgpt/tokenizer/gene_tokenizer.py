...
def tokenize_batch(
    ...
    df_co_expression: pd.DataFrame = None,
    attention_mask_file: str = 'data.pkl',
    ######################### add graph ######################
    input_g_all=None,
    ##########################################################
    ...
    #     print("Attention Mask file does not exist, need to create or populate data")
    #     df_co_expression = df_co_expression
    # else:
    #     df_co_expression = None
    ...
    for i in tqdm(range(len(data)),desc='Tokenizing data...'):
        ...
        if include_zero_gene:
            values = row
            genes = gene_ids
            if input_g_all is not None:
                idx = np.arange(len(genes))
                ########################### add graph ##################################
                tokenized_graph.append(input_g_all.subgraph(torch.from_numpy(np.insert(idx+1,0,0)).long()))
                ########################### add graph ##################################
        ...
        # if isinstance(df_co_expression, pd.DataFrame):
        #     result.append(get_attention_mask(genes.tolist(), df_co_expression, cls_id))
    ...
    # def pad_to_same_shape_attn_mask(result):
    #     """
    #     Function to pad a list of arrays to the same shape based on the maximum side length.
    #
    #     Args:
    #         result (list): List of arrays to be padded.
    #
    #     Returns:
    #         np.ndarray: Numpy array obtained by concatenating the padded arrays along the 0-axis.
    #     """
    #     max_side = 0
    #     for arr in result:
    #         max_side = max(max_side, arr.shape[0])
    #
    #     arrays = []
    #     for arr in result:
    #         diff = max_side - arr.shape[0]
    #         pad = np.pad(arr, pad_width=((0, diff), (0, diff)), mode='constant')
    ...
    # if isinstance(df_co_expression, pd.DataFrame):
    #
    #     attn_mask = pad_to_same_shape_attn_mask(result)
    #
    #     print('Attention Mask is saving...')
    #     start1 = time.time()
    #     np.save(attention_mask_file,attn_mask)
    #     end1 = time.time()
    #     t1 = (end1 - start1)*1000
    #     print('Attention Mask is Saved!')
    #     print(f'Total time: {t1} seconds')

    return tokenized_data, tokenized_graph
...
def pad_batch(
    ...
    for i in tqdm(range(len(batch)),desc='Padding batch...'):
        gene_ids, values, mod_types = batch[i]
        if len(input_g_all) != 0:
            graph = input_g_all[i]
        ...
        if len(gene_ids) > max_len:
            ...
            if len(input_g_all) != 0:
                ########################### add graph ##################################
                graph = graph.subgraph(torch.from_numpy(idx).long())
                ########################################################################
        ...
        if len(gene_ids) < max_len:
            if len(input_g_all) != 0:
                ########################### add graph ##################################
                # in advance: since when gene_ids is changed, the number will be zero
                graph.add_nodes(max_len - len(gene_ids))
                ########################################################################
            gene_ids = torch.cat(
                [
                    gene_ids,
    ...
    if len(input_g_all) != 0:
        batch_padded = {
            ...
            "graph": dgl.batch(gene_graph_list)
        }
    else:
        batch_padded = {
            "genes": torch.stack(gene_ids_list, dim=0),
...
def get_attention_mask(gene_ids_filter, df_co_expression, cls_id):
    """
    Generates an attention mask based on filtered gene IDs and co-expression data.

    Args:
        gene_ids_filter (list): List of gene IDs to filter the co-expression data.
        df_co_expression (pandas.DataFrame): DataFrame containing co-expression data.
        cls_id (str): ID of the target gene.

    Returns:
        list: Attention mask as a list of [source, target] pairs.
    """
    temp = df_co_expression[['source', 'target']]
    temp = temp[(temp['source'].isin(gene_ids_filter)) & (temp['target'].isin(gene_ids_filter))]
    new_rows = [{'source': cls_id, 'target': gene_ids_filter[i]} for i in range(len(gene_ids_filter))]
    temp = temp.append(new_rows, ignore_index=True)
    idx2gene = dict(zip(gene_ids_filter, range(len(gene_ids_filter))))
    temp['source'] = temp['source'].map(idx2gene)
    temp['target'] = temp['target'].map(idx2gene)

    M = np.zeros((len(gene_ids_filter),len(gene_ids_filter)))
    row = temp['source'].values.astype(int)
    col = temp['target'].values.astype(int)
    M[row, col] = 1
...
def tokenize_and_pad_batch(
    ...
    tokenized_data, tokenize_graph = tokenize_batch(
        data,
        gene_ids,
        return_pt=return_pt,
    ...
    batch_padded = pad_batch(
        ...
        vocab_mod=vocab_mod,
    ######################### add graph ######################
        input_g_all = tokenize_graph,
    ##########################################################
    )

    # attn_mask = np.load(attention_mask_file)
    # attn_mask_tensor = torch.from_numpy(attn_mask)
    # attn_mask_tensor.requires_grad_(False)
    # batch_padded['attn_mask'] = attn_mask_tensor
