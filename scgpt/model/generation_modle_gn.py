...
class TransformerGenerator(nn.Module):
    ...
    def forward(
        ...
        transformer_output = self._encode(
            src, values, input_pert_flags, g, src_key_padding_mask
        )
        output = {}
    ...
    def pred_perturb(
        ...
        if include_zero_gene in ["all", "batch-wise"]:
            ...
            pred_gene_values = torch.zeros_like(ori_gene_values, device=device)
            ########################## drop cls ######################
            pred_gene_values[:, input_gene_ids[:-1]] = output_values[:,:-1]
            ########################## drop cls ######################
