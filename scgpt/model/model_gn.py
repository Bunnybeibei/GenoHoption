...
class TransformerModel(nn.Module):
    def __init__(
        ...
        fast_transformer_backend: str = "gn",
        pre_norm: bool = False,
        ################# add diffusion_mode #####################
        diffusion_mode: int=0,
        ###########################################################
        ################# add hyper params #####################
        alpha: float = 0.1,
        iter_num: int = 5,
        ...
        if use_fast_transformer:
            if fast_transformer_backend == "linear":
            ...
            elif fast_transformer_backend == "gn":
                ...
                config = {
                    ...
                    'intermediate_size': d_hid,  # d_hid
                    'hidden_act': 'relu',  # activation
                    'chunk_size_feed_forward': 0,
                    'diffusion_mode':diffusion_mode, # diffusion_mode
                    'alpha':alpha,
                    'iter_num':iter_num,
                ...
                #############################################################
                self.transformer_encoder = gn_transformer_encoder(gn_layer, config, nlayers)
        else:
    ...
    def _encode(
        ...
        output = self.transformer_encoder(
            total_embs, g=g, attention_mask=src_key_padding_mask
        )
        ############ encoder embedding ####################
    ...
    def forward(
        ...
        transformer_output = self._encode(
            src, values, g, src_key_padding_mask, batch_labels
        )
        if self.use_batch_labels:
            batch_emb = self.batch_encoder(batch_labels)  # (batch, embsize)
    ...
    def encode_batch(
        ...
        for i in range(0, N, batch_size):
            ...
            for j in range(i,i+batch_size):
                input_g.append(dgl.slice_batch(g, j))

            input_g = dgl.batch(input_g)
...
class MHA(nn.Module):
    ...
    def attention(self, query, key, value, mask=None, dropout=None):
        ...
        if mask is not None:
            scores = scores.masked_fill(mask == 0, -1e9)  # Apply the mask to attention scores
