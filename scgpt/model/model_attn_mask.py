...
class TransformerModel(nn.Module):
    ...
    def forward(
        ...
        transformer_output = self._encode(
            src, values, src_key_padding_mask, batch_labels, attn_mask=attn_mask,
        )
        if self.use_batch_labels:
            batch_emb = self.batch_encoder(batch_labels)  # (batch, embsize)
...
class MHA(nn.Module):
    def __init__(self, num_heads, embed_dim, attention_dropout=0., bias=True, device=None, dtype=None):
        """
        MHA module performs multi-head attention on input sequence.
        Args:
        ...
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias=bias,
                                  **factory_kwargs)  # Linear transformation for output projection
        self.dropout = nn.Dropout(p=attention_dropout)  # Dropout layer for regularization

    def attention(self, query, key, value, attn_mask=None, dropout=None):
        "Attention computation"
        scale = 1 / np.sqrt(query.shape[-1])  # Compute the scale factor for attention scores
        query = query * scale  # Scale the query
        scores = torch.einsum('...i d, ...j d -> ...i j', query, key)  # Compute attention scores using matrix multiplication
        if attn_mask is not None:
            scores = scores.masked_fill(attn_mask == 0, -1e9)  # Apply the mask to attention scores
        p_attn = F.softmax(scores, dim=-1)  # Compute attention probabilities using softmax
        if dropout is not None:
            p_attn = dropout(p_attn)  # Apply dropout to attention probabilities
        return torch.matmul(p_attn, value), p_attn  # Return the weighted sum of values and attention probabilities

    def forward(self, x, attn_mask=None, key_padding_mask=None):
        """
            x: (batch, seqlen, hidden_dim) (where hidden_dim = num heads * head dim)
            mask: bool tensor of shape (batch, seqlen, seqlen)
        """

        qkv = self.Wqkv(x)
        qkv = rearrange(qkv, 'b s (three h d) -> b s three h d', three=3, h=self.num_heads)

        q,k,v = qkv[:, :, 0, :, :].permute(0,2,1,3), \
                qkv[:, :, 1, :, :].permute(0,2,1,3), \
                qkv[:, :, 2, :, :].permute(0,2,1,3)

        x, self.attn = self.attention(q, k, v, mask=attn_mask, dropout=self.dropout)

        x = x.permute(0, 2, 1, 3)
        return self.out_proj(rearrange(x, 'b s h d -> b s (h d)')), self.attn
...
class FlashTransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if check_model:
            ...
            self.self_attn_2.Wqkv = self.self_attn.Wqkv
            self.self_attn_2.out_proj = self.self_attn.out_proj
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.norm_scheme == "pre":
            ...
            if self.check_model:
                """
                When atol is greater than or equal to 1e-3, and dropout rate is 0, the outputs can be considered equal.
                """
                a1 = self.self_attn(src, key_padding_mask=src_key_padding_mask_)[0]
                a2 = self.self_attn_2(src, key_padding_mask=src_key_padding_mask_)[0]
                assert torch.allclose(a1, a2, atol=1e-3), "check fail"
                src2 = a2
            else:
                src2 = self.self_attn(src, key_padding_mask=src_key_padding_mask_, attn_mask=attn_mask)[0]
            src = src + self.dropout1(src2)
            src = self.norm1(src)
            src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
            src = src + self.dropout2(src2)
