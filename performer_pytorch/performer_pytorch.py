...
def softmax_kernel(data, *, projection_matrix, is_query, normalize_data=True, eps=1e-4, device = None):
    ...
    data_dash = torch.einsum('...id,...jd->...ij', (data_normalizer * data), projection.clone())
...
class FastAttention(nn.Module):
    ...
    def forward(self, q, k, v, output_attentions = False):
        ...
        # print(f"{q.shape[2]} 's time is {execution_time} s")
    ...
    def forward(self, x, pos_emb = None, context = None, mask = None, context_mask = None, output_attentions = False, **kwargs):
        ...
        if not empty(q):
            ...
            if output_attentions:
                ...
                max_mem = torch.cuda.max_memory_allocated()
                execution_time = end_time - start_time
                print(f"{out.shape[2]} 's time is {execution_time} s")
                print(f'Max memory used by method: {max_mem / (1024 ** 3)} GB')
                ##############################################################################
            else:
...
class SinExpressionEmbedding(nn.Module):
    def __init__(self, d_model: int, dropout: float = 0.1, wavelength: float = 12.0):
        super().__init__()
        self.d_model = d_model
        self.dropout = nn.Dropout(p=dropout)

        div_term = torch.exp(torch.arange(0, d_model, 2) * (-math.log(wavelength) / d_model))
        # Add dimensions for easy multiplication with input tensors
    ...
    def forward(self, x):
        ...
        pe[:,:,0::2] = torch.sin(x[:,:,None] * self.div_term)
        pe[:,:,1::2] = torch.cos(x[:,:,None] * self.div_term)
        return self.dropout(pe)

class Gene2VecPositionalEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len, g2v_file):
        super().__init__()
...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        g2v_file = None,
        sin_emb_wavelength = 12.0,
        auto_check_redraw = True,
        qkv_bias = False,
        pred_continuous = False           # False: predict the categorical bucketed expression, True: predict the continuous expression value
    ):
        super().__init__()
        local_attn_heads = cast_tuple(local_attn_heads)
        ...
        if(pred_continuous):
            self.token_emb = SinExpressionEmbedding(dim, wavelength = sin_emb_wavelength)
        else:
            self.token_emb = nn.Embedding(num_tokens, dim)

        if g2v_position_emb:
            self.pos_emb = Gene2VecPositionalEmbedding(dim, max_seq_len, g2v_file)
    ...
    def forward(self, x, return_encodings = False, output_attentions = False, **kwargs):
        ...
        x = self.token_emb(x)     # expression level
        if output_attentions:
            x.requires_grad_()    # used for attn_map output
        x += self.pos_emb(x)      # add gene embedding (gene2vec or zeroes)
        x = self.dropout(x)

        # performer layers
        layer_pos_emb = self.layer_pos_emb(x) #this returns None (Rebecca)
        if output_attentions:
            # ####################### watch memory #########################################
            ...
            # print(f'501 GPU memory used by method: {(mem_after - mem_before) / (1024 ** 3)} GB')
            # ##############################################################################
            # norm and to logits
            x = self.norm(x)
            if return_encodings:
                return self.to_out(x), x, attn_weights
            ...
            if exists(self.to_out):
                x = self.to_out(x)
                return x #batch size x seq len x hidden (logits)

            return x @ self.token_emb.weight.t() #batch size x seq len x vocab size
