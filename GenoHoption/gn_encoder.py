...
class gn_transformer_encoder(nn.Module):
    def __init__(self, encoder_layer, config, num_layers, gradient_checkpointing=False):
        super().__init__()
        self.layer = nn.ModuleList([encoder_layer(config, i) for i in range(num_layers)])
