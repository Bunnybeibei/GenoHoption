...
from utils import create_adj_mat, from_adj_to_batched_graphs, pad_to_window_size
...
FRAC = 0.75
for mode in ['bigbird', 'Longformer', 'window']:
    for DATASET_NAME in ['ms', 'pancreas', 'myeloid']:
        for SEED in [0, 1, 2, 3, 4]:

            if DATASET_NAME == 'ms' and mode != 'bigbird':
                continue
            ...
            hyperparameter_defaults = dict(
                ...
                dab_weight=0.0,
                lr=1e-4,
                batch_size=32, #if mode != 'bigbird' else 16,
                layer_size=128,
                ...
                diffusion_mode=0,
                iter_num=6 if mode=='window' else 1,
                alpha=0.25 if mode=='window' else 0.,
            )
            ...
            if dataset_name == "ms":
                ...
                averge_degree = 64 if mode=='Longformer' else 14
            ...
            if dataset_name == "pancreas": #RB
                ...
                adata_test_raw = adata_test.copy()
                adata = adata.concatenate(adata_test, batch_key="str_batch")
                averge_degree = 64 if mode=='Longformer' else 22
            ...
            if dataset_name == "myeloid":
                ...
                filter_gene_by_counts = False
                adata = adata.concatenate(adata_test, batch_key="str_batch")
                averge_degree = 64 if mode=='Longformer' else 34
            ...
            if fraction != 1:
                ...
                all_counts, _, celltypes_labels, _, batch_ids, _ = train_test_split(all_counts, celltypes_labels, batch_ids, train_size=fraction, random_state=config.seed, shuffle=True, stratify=celltypes_labels)
            ...
            src_dst = create_adj_mat(attention_window=averge_degree,
                                     max_len=2048,
                                     mode=mode)
            def train(model: nn.Module, loader: DataLoader) -> None:
                ...
                for batch, batch_data in enumerate(loader):
                    ...
                    batch_data, src_key_padding_mask = pad_to_window_size(averge_degree, batch_data, pad_token_id=vocab[pad_token], src_key_padding_mask=src_key_padding_mask)
